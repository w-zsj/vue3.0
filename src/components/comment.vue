  ...
  <div class="comment-box padding-15">
    ...
    <div v-if="commentcount > 0">
      <div class="commentList">
        <div class="comment-item" v-for="(commentItem,commentIndex) in comment.list" :key="commentIndex">
          <div class="user-pic" @click="callApp">
          ...
          <div class="comment-detl">
            <div class="name-attitude">
              ...
              <div class="user-attitude" @click="callApp">
                <span v-if="commentItem.favcount>0">{{commentItem.favcount}}</span>
                <ico :name="commentItem.favcount?'posting-like-small':'posting-like-gray-small'"></ico>
              </div>
            </div>
            <div class="comment-info">{{commentItem.comment}}</div>
            <div class="date-reply">
              <div>{{commentItem.createtime | formatDate}}</div>
              <div class="reply-num">{{commentItem.replys.length}}  回复</div>
            </div>
            <div class="replyBox" v-if="commentItem.replys && commentItem.replys.length > 0">
              <div class="reply-item" :class="{'last-child': replyIndex == commentItem.replys.length-1}" v-for="(replyItem,replyIndex) in commentItem.replys" :key="replyIndex" v-show="replyIndex<2">
                <div class="reply-user">
                  <span class="user_name">{{replyItem.nickname}}</span>
                  <span>：{{replyItem.comment}}</span>
                </div>
              </div>
              <div @click="callApp" class="replyBottom" v-if="commentItem.replys && commentItem.replys.length > 2">
      ...
      <div @click="callApp" class="bottomBox" v-if="commentcount > 3">
        <span>展开余下{{commentcount-3}}条评论</span>
      </div>
      <div class="end flex-center" v-else-if="commentcount>0 && commentcount<=3">— THE END —</div>
    </div>
    <div v-else>
      <NoContent :sourceType="4"></NoContent>
    </div>
  </div>
</template>
<script >
...
export default {
  ...
  filters: {
    // x（<60）分钟前、x（<24）小时前、x（<3）天前、xx-xx（日期）
    formatDate: val => DateDiffFormat(val, { d: 3, dShow: 'yyyy-MM-dd' })
  }
}
</script>
